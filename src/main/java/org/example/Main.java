...
public class Main {
    ...
    public static double safeDivision(double a, double b) {
        // Handle the unchecked ArithmeticException by returning 0.
        try {
            return divide(a, b);
    ...
    public static String getSecondLetter(String s, List<String> logMessages) {
        // Handle the StringIndexOutOfBoundsException by returning an empty string ("")
        try {
            return s.substring(1, 2);
        } catch (StringIndexOutOfBoundsException e) {
            return "";
        } finally {
            // This line needs to happen after the try/catch block, regardless of whether an exception is thrown (put it in a finally block)
    ...
    public static double safeDivideWithCustomException(double a, double b) throws CustomDivideByZeroException {
        // handle divide by zero by throwing a CustomDivideByZeroException
        // Modify the custom exception to pass a,b to the constructor and store them in the exception (see CustomDivideByZeroException.java)
            if (b == 0 ) {
